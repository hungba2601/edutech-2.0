...
interface PensionCalculatorBHXHModalProps {
  isOpen: boolean;
  onClose: () => void;
  onLogin?: () => void;
  videoUrl?: string;
...
export const PensionCalculatorBHXHModal: React.FC<PensionCalculatorBHXHModalProps> = ({ isOpen, onClose, onLogin, videoUrl }) => {
  const [copied, setCopied] = useState(false);
  const appLink = 'https://tinh-che-do-huu-bhxh.vercel.app/';
            ...
            <div>
              <div className="flex items-center space-x-2">
                <h2 className="text-xl sm:text-2xl font-black font-outfit uppercase tracking-wider drop-shadow-sm">
                  CÔNG CỤ TÍNH CHẾ ĐỘ LƯƠNG THEO LUẬT BHXH
                </h2>
              ...
              <p className="text-blue-50 opacity-90 text-sm font-medium mt-0.5">
                Công cụ giúp tính chế độ lương hưu theo luật BHXH
              </p>
            </div>
          </div>
          <button 
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6 p-5 bg-white dark:bg-slate-800 rounded-3xl shadow-md border border-blue-100 dark:border-slate-700/50">
            ...
            <div className="space-y-2 text-center sm:text-left">
              ...
              <p className="text-slate-700 dark:text-slate-300 font-bold leading-relaxed text-sm md:text-base text-justify">
                Công cụ giúp tính chế độ lương hưu theo luật BHXH. Đảm bảo tính chính xác cao, dễ dàng sử dụng và hoàn toàn miễn phí.
              </p>
            </div>
          </div>

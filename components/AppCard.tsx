  ...
  return (
    ...
    >
      ...
      <div className="absolute top-3 right-3 sm:top-4 sm:right-4 z-20 flex flex-col items-end space-y-1">
        ...
        {/* NEW Badge */}
        {app.isNew && (
          <span className={`${(app.id === 'skkn-ai' || app.id === 'biology-sim' || app.id === 'learning-assistant' || app.id === 'ai-virtual-assistant' || app.id === 'text-to-speech' || app.id === 'pdf-to-word' || app.id === 'slide-assistant' || app.id === 'word-to-katex' || app.id === 'exam-shuffler-ai' || app.id === 'pdf-editor-nph') ? 'bg-red-600 shadow-red-200' : 'bg-gradient-to-r from-emerald-500 to-teal-400 shadow-emerald-200'} text-white text-[9px] sm:text-[10px] font-black px-2 sm:px-3 py-0.5 sm:py-1 rounded-full shadow-lg animate-pulse uppercase tracking-wider`}>
            NEW
          </span>
        )}
        ...
        {/* GIFTED Badge */}
        {app.isGifted && (
          <span className="bg-gradient-to-r from-pink-600 to-rose-500 text-white text-[9px] sm:text-[10px] font-black px-2 sm:px-3 py-0.5 sm:py-1 rounded-full shadow-lg shadow-rose-200 animate-pulse uppercase tracking-wider">
            TẶNG KÈM
          </span>
        )}
      </div>

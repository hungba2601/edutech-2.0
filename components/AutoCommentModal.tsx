...
export const AutoCommentModal: React.FC<AutoCommentModalProps> = ({ isOpen, onClose, onDownload, videoUrl }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-opacity">
      ...
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-emerald-600 to-teal-600 p-6 flex items-center justify-between text-white relative">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-emerald-50 opacity-90 text-sm font-medium">TIỆN ÍCH TỰ ĐỘNG NHẬN XÉT HK & HỌC BẠ CÁC MÔN HỌC</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="overflow-y-auto p-8 space-y-6 bg-white dark:bg-slate-900 transition-colors duration-300">
          <div className="space-y-4">
            <div className="flex items-start space-x-3 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
              <div className="flex-shrink-0 w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-600 dark:text-emerald-400 font-bold">1</div>
            ...
            <div className="flex items-start space-x-3 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
              <div className="flex-shrink-0 w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-600 dark:text-emerald-400 font-bold">2</div>
              <p className="text-slate-700 dark:text-slate-300 font-medium text-justify">
                Xem video Hướng dẫn để thực hiện cài đặt tiện ích lên trình duyệt và cách cấu hình nhận xét tự động theo từng môn học.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 flex flex-col sm:flex-row items-center justify-center gap-4 transition-colors">
          <button 
            onClick={onDownload}
            className="flex items-center justify-center space-x-2 w-full sm:w-auto px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow-lg transition-all active:scale-95 uppercase text-sm"
          >
            <Download size={18} />
